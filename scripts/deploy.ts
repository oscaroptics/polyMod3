...
const BASE_PATH = "./circuits/oscarMultiplier/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/oscarMultiplier.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/OscarMultiplierVerifier.sol:Verifier");
